...
Basisstapel::Basisstapel(const QPixmap& pixmap, Scene* oparent, QGraphicsItem* gparent) : QObject(oparent), QGraphicsPixmapItem(pixmap, gparent), meine_scene(oparent), meinrahmen(0), meinhilfszeiger(0), bild(pixmap), bild_skaliert(pixmap), stapelgroesse(pixmap.height())
{
...
bool Basisstapel::lege_karte_ab(Karte* karte)
{
    ...
    if (ablage_moeglich(karte) == true)
    {
        ...
        // im austeilstapel und austeilcostapel muss der zug aufgrund der besonderheiten seperat gesendet werden
        if (objectName() != BASISSTAPEL_AUSTEILCOSTAPEL && objectName() != BASISSTAPEL_AUSTEILSTAPEL) emit zug(Zug(karte, quelle, this));
...
void Basisstapel::initialisiere_karte(Karte* karte)
{
    ...
    // sicherheitshalber die karte nach hause schicken. dies muss ganz am ende der initialisierung geschehen, damit auf keinen fall noch 0 zeiger in der karte vorhanden sind !
    karte->nach_hause();
}
...
{
    // im austeilstapel und austeilcostapel muss der zug aufgrund der besonderheiten seperat gesendet werden
    if (objectName() != BASISSTAPEL_AUSTEILCOSTAPEL && objectName() != BASISSTAPEL_AUSTEILSTAPEL) emit zug(Zug(karte, this));
}
...
{
    QRectF ueberlappung_rect(gesamt_rect().intersected(karte->gesamt_rect()));
...
void Basisstapel::hilfsanfrage_start(Karte* karte)
{
    ...
    if (ablage_moeglich(karte) == true)
    {
        // die position des hilfszeiger setzen und ...
...
void Basisstapel::karten_auffaechern()
{
}


Scene* Basisstapel::meineScene() const
{
    return meine_scene;
}
