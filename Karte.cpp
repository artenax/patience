...
Karte::Karte(const QPixmap& vorderseite, const QPixmap& hinterseite, Scene *oparent, QGraphicsItem *gparent) : QObject(oparent), QGraphicsPixmapItem(hinterseite, gparent), vorne(vorderseite), hinten(hinterseite), vorne_skaliert(vorderseite), hinten_skaliert(hinterseite), Wert(0), vorderseite_oben(false), rahmen_anzeigen(true), double_click_sperre(false), transparenz(true), scene(oparent), kreutzstapel(0), pikstapel(0), herzstapel(0), karostapel(0), kartengroesse(vorderseite.height())
{
    setAcceptedMouseButtons(Qt::LeftButton | Qt::RightButton);

    // den transparenz effekt fuer die karte erzeugen
    transparenz_effekt = new QGraphicsOpacityEffect(this);
    setGraphicsEffect(transparenz_effekt);

    // den transparenz effekt zunaechst abschalten. er wird nur benoetigt, wenn die karte aufgenommen wird
    transparenz_effekt->setEnabled(false);
}
...
void Karte::zeige_vorderseite()
{
    ...
    // wenn es sich beim stapel nicht um austeilcostapel handelt oder es sich um austeilcostapel handelt und nur eine karte gezogen wird  die karte beweglich machen
...
{
    if (event->buttons() == Qt::LeftButton && (scene->aktiveKarte() == 0 || scene->aktiveKarte() == this) && flags() == QGraphicsItem::ItemIsMovable && event->button() == Qt::LeftButton && ist_vorderseite() == true && Meinstapel->ist_oberste_karte(this) == true && hat_kinderkarten() == false && Meinstapel->objectName().contains(BASISSTAPEL_ZIELSTAPEL) == false)
    {
...
{
    // linke maustaste
    if (event->button() == Qt::LeftButton && scene->aktiveKarte() == 0)
    {
        scene->setze_aktive_karte(this);
    ...
    else if (event->button() == Qt::RightButton && flags() == QGraphicsItem::ItemIsMovable && scene->aktiveKarte() == 0)
    {
        scene->setze_aktive_karte(this);
...
void Karte::mouseReleaseEvent(QGraphicsSceneMouseEvent* event)
{
    ...
    if (event->buttons() == Qt::NoButton)
    {
        ...
        if (double_click_sperre == false)
        {
            ...
            // die ist noetig, weil beim aufdecken auf dem austeilcostapel die karte unbeweglich gemacht wurde. dies ist jedoch nur zulaessig, wenn die karte die oberste karte ist.
            if (Meinstapel->objectName() == BASISSTAPEL_AUSTEILCOSTAPEL && Meinstapel->ist_oberste_karte(this) == true) setFlag(QGraphicsItem::ItemIsMovable, true);
        }
...
{
    // nur, wenn die karte die momentan aktive karte ist
    if (scene->aktiveKarte() == this)
    {
        if (double_click_sperre == false && event->buttons() == Qt::LeftButton)
        {
            // wenn karte beweglich ist transparenz an
            if (flags() == QGraphicsItem::ItemIsMovable && transparenz == true) transparenz_effekt->setEnabled(true);
...
bool Karte::lade(const QStringList& daten)
{
    ...
    if (daten.size() == KARTE_ANZAHL_SPEICHERELEMENTE && daten.first() == objectName())
    {
        ...
        if (daten.at(KARTE_IST_VORNE_IDX).toInt() == 1)
        {
            zeige_vorderseite();

            // wenn nur eine karte gezogen wird und die karte auf austeilcostapel liegt sicherstellen, das sie beweglich ist
            if (eigentuemer_stapel()->objectName() == BASISSTAPEL_AUSTEILCOSTAPEL && scene->nur_eine_wird_gezogen() == true) setFlag(QGraphicsItem::ItemIsMovable, true);

            // wenn drei karten gezogen werden und die karte auf austeilcostapel liegt die karte nur dann beweglich machen, wenn sie die oberste karte ist
            else if (eigentuemer_stapel()->objectName() == BASISSTAPEL_AUSTEILCOSTAPEL && scene->nur_eine_wird_gezogen() == false && eigentuemer_stapel()->ist_oberste_karte(this) == true) setFlag(QGraphicsItem::ItemIsMovable, true);
        }
...
{
    // die karten groesse speichern
    kartengroesse = wert;

    // hinter und vorderseite der karte der groesse entsprechend skalieren
    ...
    // die neu skalierten bilder uebernehmen
...
void  Karte::nach_hause()
{
    ...
    // die karte auf die rueckkehrposition schicken
    setPos(Rueckkehrkoordinaten);

    // den zwert des stapels, zu dem die karte gehoert normalisieren
    Meinstapel->normalisiere_zwert();

    // den rahmen wieder verstecken
    emit rahmen_verstecken();

    // die hilfsanfrage auf jeden fall auch mal beenden
    emit hilfsanfrage_ende();

    // wenn die karte noch gegrabbt ist, sie ungrabben
...
{
    // die aktuelle position als rueckkehrposition gespeichert wird
    Rueckkehrkoordinaten = pos();
}


void Karte::setze_transparenz_effekt(bool wert)
{
    transparenz = wert;

    // wenn der transparenz effekt abgeschaltet wird sicherstellen, das die transparenz sofort deaktiviert wird.
    if (wert == false) transparenz_effekt->setEnabled(false);
}
