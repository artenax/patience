...
Punktezaehler::Punktezaehler(Scene *parent) : QObject(parent), punkte(0), scene(parent)
{
...
void Punktezaehler::reaktion_auf_timeout()
{
    if (punkte > 0)
    {
        if (scene->nur_eine_wird_gezogen() == true) punkte -= PUNKTEZAEHLER_EINE_ZIEHEN_PUNKTEABZUG_ZEIT;
...
void Punktezaehler::stapel_durch()
{
    if (punkte > 0)
    {
        if (scene->nur_eine_wird_gezogen() == true) punkte -= PUNKTEZAEHLER_EINE_ZIEHEN_AUSTEILSTAPEL_DURCH;
...
int Punktezaehler::kalkuliere_punkte(const Zug& zug) const
{
    ...
    if (zug.ist_bewegung() == true)
    {
        ...
        {
            if (scene->nur_eine_wird_gezogen() == true) erg += PUNKTEZAEHLER_EINE_ZIEHEN_AUSTEILSTAPEL_ZU_ABLAGESTAPEL;
        ...
        else if (zug.herkunft_name() == BASISSTAPEL_AUSTEILCOSTAPEL && zug.ziel_name().contains(BASISSTAPEL_ZIELSTAPEL) == true && nach_oben_liste.contains(zug.karte_name()) == false)
        {
            ...
            if (scene->nur_eine_wird_gezogen() == true) erg += PUNKTEZAEHLER_EINE_ZIEHEN_AUSTEILSTAPEL_ZU_ZIELSTAPEL;
        ...
        {
            if (scene->nur_eine_wird_gezogen() == true) erg += PUNKTEZAEHLER_EINE_ZIEHEN_ABLAGESTAPEL_ZU_ZIELSTAPEL;
    ...
    else if (zug.ist_aufdeckgung() == true)
    {
        ...
        {
            if (scene->nur_eine_wird_gezogen() == true) erg += PUNKTEZAEHLER_EINE_ZIEHEN_KARTE_AUF_ABLAGESTAPEL_AUFGEDECKT;
