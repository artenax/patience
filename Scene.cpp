...
Scene::Scene(const QString& deckblatt, QObject *parent) : QGraphicsScene(parent), Laufendes_spiel(false), nur_eine_ziehen(true), aktive_karte(0)
{
    ...
    for (register int idx = 0; idx < kartenliste.size(); idx++)
    {
        ...
        connect(this, SIGNAL(setze_transparenz(bool)), kartenliste.at(idx), SLOT(setze_transparenz_effekt(bool)));
    }
    ...
    connect(this, SIGNAL(neues_spiel_gestartet()), austeilcostapel, SLOT(alle_karten_einreihen()));

    // alle stapel an undo anschliessen
    for (register int idx = 0; idx < allestapel.size(); idx++)
    {
        connect(allestapel.at(idx), SIGNAL(zug(const Zug&)), undo, SLOT(speichere_zug(const Zug&)));
    }
...
Karte* Scene::aktiveKarte() const
{
    return aktive_karte;
}


void Scene::setze_aktive_karte(Karte* karte)
{
    aktive_karte = karte;
}


void Scene::initialisiere_transparenz(bool wert)
{
    setze_transparenz(wert);
}
