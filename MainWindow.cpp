...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent)
{
    ...
    // unter macos soll action_beenden unsichtbar sein
#ifdef Q_OS_MAC

    action_beenden->setVisible(false);
    ...
    // das steuern der transparenz ermoeglichen
    connect(action_transperenz, SIGNAL(triggered(bool)), scene, SIGNAL(setze_transparenz(bool)));
...
{
    QMessageBox::about(this, tr("About Patience"), tr("Patience version %1 \n\nAuthor:\tAndreas Konarski\nLicence:\tgpl v3 or later\n\nContact:\n\nprogrammieren@konarski-wuppertal.de\nwww.konarski-wuppertal.de\n\nAbout the game cards:\n\nThis PySol cardset was adapted from the game XSkat 4.0\nCopyright (C) 2004 Gunter Gerhardt (http://www.xskat.de)\nThis cardset is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 2 of the License, or (at your option) any later version.").arg(VERSION));
}
...
void MainWindow::restliche_einstellungen_laden()
{
    ...
    // einstellungen fuer die transparenz laden
    action_transperenz->setChecked(einstellungen->value("mainwindow/transparenz", true).toBool());
    scene->initialisiere_transparenz(action_transperenz->isChecked());
}
...
void MainWindow::einstellungen_speichern()
{
    ...
    // einstellungen fuer die transparenz speichern
    einstellungen->setValue("mainwindow/transparenz", action_transperenz->isChecked());
}
